...
The following example generates a table of all route prefixes for the equivalent netmask ``IPAddress`` input.
...
.. code-block:: c#
   ...
   :name: NetmaskToCidrRoutePrefix Example
    ...
    public void NetmaskToCidrRoutePrefix_Example()
    {
        ...
        foreach (var routePrefix in Enumerable.Range(0, 33))
        {
            var subnet = new Subnet(IPAddress.Parse("0.0.0.0"), routePrefix);
            var netmask = subnet.Netmask;

            _ = output.Append(routePrefix)
                      .Append('\t')
...
``ToHexString`` may be used to encode the ``IPAddress ipAddress`` as a big-endian ordered hexadecimal string. It will keep all zero-valued most significant bytes.
